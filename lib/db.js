...
exports.findFileCounter = (value) => {
  ...
  let _sql = `select count(*) as total_number from files where 1=1`;
  if (use_id) {
...
exports.fileEditName = (value) => {
  let _sql = `update files set file_user_name=? where file_user_id=?;`;
  return query(_sql, value);
};
...
exports.fileMultipleFind = (value) => {
  let _sql = `select * from files where file_user_id=?`;
  ...
  if (time_range) {
    ...
    _sql += " and file_upload_time between  Date(?) and Date(?)";
  }

  // 排序操作
  _sql += " order by file_upload_time desc";
...
// 移动端接口
// 1.查询文件
exports.m_searchfile = (value) => {
  const cleanedArray = value.map((item) => "%" + item + "%");
  console.log(cleanedArray);

  let _sql = `select * from files where file_name like ? or file_remark like ? order by file_upload_time desc limit 20`; // and file_type=8;
  return query(_sql, cleanedArray);
};
